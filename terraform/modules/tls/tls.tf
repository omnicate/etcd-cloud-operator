...
resource "tls_cert_request" "clients-server" {
  ...
  key_algorithm   = tls_private_key.clients-server[count.index].algorithm
  private_key_pem = tls_private_key.clients-server[count.index].private_key_pem
...
resource "tls_locally_signed_cert" "clients-server" {
  ...
  cert_request_pem      = tls_cert_request.clients-server[count.index].cert_request_pem
...
resource "tls_cert_request" "clients" {
  ...
  key_algorithm   = tls_private_key.clients[count.index].algorithm
  private_key_pem = tls_private_key.clients[count.index].private_key_pem
...
resource "tls_locally_signed_cert" "clients" {
  ...
  cert_request_pem      = tls_cert_request.clients[count.index].cert_request_pem
  ca_key_algorithm      = local.ca["alg"]
  ca_private_key_pem    = local.ca["key"]
  ca_cert_pem           = local.ca["cert"]
  validity_period_hours = 8760
